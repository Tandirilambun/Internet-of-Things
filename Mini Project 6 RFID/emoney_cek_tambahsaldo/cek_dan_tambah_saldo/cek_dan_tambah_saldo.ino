...
void setup() {
    ...
    Serial.println("E-MONEY Tanpa Server Cek dan Tambah Saldo");
...
void loop() {
  if (Serial.available()){
    isiSaldo = true;
    //untuk menampung inputan saldo yang di masukkan
    input = "";
    while(Serial.available()>0){
      input += char(Serial.read());
    }
    //Serial.println(input);
    //mengubah saldo ke int karena di atas menggunakan string
    saldo = input.toInt();
    //saldo hanya maksimal 255 karena hanya menggunakan 1 block address
    if (saldo > 255){
      saldo = 0;
    ...
    digit = saldo;
    //saldo yang diinputkan dikalikan dengan 1000
    saldo *= 1000;
  ...
  if (notif){
    ...
    Serial.println("contoh input 9 || max saldo 255rb");
    Serial.println();
  ...
  if (isiSaldo){
    ...
    saldo = buffer[0];
    saldo *= 1000;
    Serial.print("===============>>>> Saldo kartu sekarang : Rp. ");
    Serial.println(saldo);
  }
